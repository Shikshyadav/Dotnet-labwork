...
namespace WindowsFormsApp1
{
    ...
    {
        public void RegisterUser()  // ✅ Correct constructor name matching class
        {
            InitializeComponent();

            // Make sure the form behaves like a user control inside the panel
            this.TopLevel = false;  // Makes the form not top-level (so it can be embedded)
            this.FormBorderStyle = FormBorderStyle.None;  // Removes the border
            this.Dock = DockStyle.Fill;  // Fills the parent container (panel)
        }
